...
- Phase 11 GitHub Copilot Best Practices: ✅ Built (2026-07-11) — Part 3 of the GitHub Copilot Enterprise Series as committed repo content; docs-only phase (no runtime, no Ollama). Enterprise Copilot blueprint (9 chapters: features, custom instructions, prompt files, custom agents, skills+hooks, MCP governance, model selection/context engineering, enterprise governance, CI/CD + code review), generic stack-agnostic `copilot_starter_kit/` (.github instructions/prompts/agents/skill + .vscode mcp.json/settings.json + AGENTS.md; all prompts use the series' 6-part Role→Context→Task→Constraints→Output→Reference shape), and stack overlays in `stacks/`: java_springboot_microservices (flagship OWASP REST API prompt), angular, react, ios_swift (Swift/ObjC/SwiftUI/UIKit). iOS overlay completed 2026-07-14 for series Parts 4–5: 11 prompts covering the 12 standard iOS prompt categories (memory-leak review folded into swift-concurrency-review), reference-repo tables with star counts verified live via GitHub API, and `stacks/ios_swift/walkthrough.md` — one banking feature carried ticket → analysis → data → UI → tests → three reviews → PR with the exact prompt per step. Overlay v2 (2026-07-21): Xcode support-matrix section (instructions + applyTo + prompt files + custom agents all work in Xcode per the extension's own Docs/, general GitHub docs lag), two read-only iOS agents (legacy-estate-analyst, ios-accessibility-auditor), iOS `mcp.json` (getsentry/XcodeBuildMCP + ios-simulator-mcp, version-pinned + allowlisted per docs/06), reference tables expanded to 6 categories / 18 repos verified live (adds fastlane, SwiftFormat, tuist, snapshot-testing, swift-testing, airbnb/swift, 3 MCP servers). Overlay v2 Tier 2+3 (same PR #30): `stacks/ios_swift/ci_ios.yml` (macOS runner, split lint/test, xcresult artifact, Linux cloud-agent caveat), `stacks/ios_swift/skills/privacy-manifest-audit/` (first iOS skill — audits PrivacyInfo.xcprivacy vs actual required-reason API usage), iOS rules added to the examples/hooks/security-scan hook (ATS/get-task-allow/plist+xcconfig secrets — tested; also fixed a placeholder-allowlist bug that exempted plist XML tags), walkthrough "step 0.5" wiring XcodeBuildMCP. awesome-copilot upstream contribution staged LOCAL-only in `launch/upstream_awesome_copilot/` (2 enterprise-neutral instruction files, submit after Part 4 publishes). Personal post drafts stay local in `launch/` (gitignored). Path: `Phase11_GitHub_Copilot_Best_Practices/`. Branch `phase11-github-copilot-best-practices`.
