...
|---|---|---|---|
| 01 | MCP Benefits Assistant | Local MCP server for mock primary contribution + savings account tools/resources | Built |
...
```text
Module 01: Learn MCP with mock primary contribution/savings account tools and resources
    ↓
...
primary contribution and savings account questions are a good teaching domain because they naturally mix:
...
The examples use mock data only. They are educational and are not professional,
adjustment, legal, or allocation advice.

## Existing Prototype

`mcp_benefits_example/` is an earlier MCP+RAG prototype that uses a retrieval
tool over local primary contribution/savings account markdown reference docs. Keep it as a reference for
