...
class BenefitsDemoAgentServiceTest {
    ...
    void routesCombinedPersonalAndPolicyQuestionToMcpAndRag() {
        ...
        assertThat(response.toolCalls())
                .extracting("name")
                .contains("get_employee_profile", "calculate_401k_match", "search_benefits_docs", "list_sources");
    ...
    void routesPolicyQuestionToRagOnly() {
        var response = service.answer("What is the 2026 HSA family contribution limit?");

        assertThat(response.route()).isEqualTo("rag");
        assertThat(response.toolCalls())
                .extracting("name")
                .containsExactly("search_benefits_docs", "list_sources");
        assertThat(response.retrievedDocuments().get(0).source()).isEqualTo("hsa_reference.md");
