...
func extractContractPayload(message Message) (map[string]interface{}, *JSONRPCError) {
	for _, part := range message.Parts {
		// Accept parts explicitly marked as data parts, or parts that carry a
		// non-nil Data map even without an explicit kind/type tag (the ADK
		// RemoteA2aAgent can send DataParts this way).
		if part.Data != nil {
			if part.Kind == "data" || part.Type == "data" || (part.Kind == "" && part.Type == "") {
