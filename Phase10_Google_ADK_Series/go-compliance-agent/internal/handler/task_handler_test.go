...
func TestHandleJSONRPCMessageSend(t *testing.T) {
	...
	if response.Result.Status.State != "completed" {
		t.Fatalf("expected completed task state completed, got %q", response.Result.Status.State)
	}
	...
	if message.Role != "agent" {
		t.Fatalf("expected message role agent, got %q", message.Role)
	}
	if len(message.Parts) != 1 || message.Parts[0].Kind != "data" || message.Parts[0].MediaType != "application/json" {
	...
	if len(response.Result.Artifacts) != 1 || response.Result.Artifacts[0].ArtifactID == "" {
		t.Fatalf("expected one artifact with artifactId, got %+v", response.Result.Artifacts)
	}
}
