...
class LiveComplianceCase:
    ...
    java_handoff: dict[str, Any] = field(default_factory=dict)
...
def _compliance_cert_html(case: LiveComplianceCase, details: dict, verdict: dict) -> str:
    ...
    verdict_caption = (
        ...
        if passed
        else f"{violation_count} policy exception{'s' if violation_count != 1 else ''} require{'s' if violation_count == 1 else ''} legal review."
    )
    if status_class == "manual":
...
<html lang="en">
  ...
  <body>
    <main>
      ...
      <h2>Go Policy Validation Summary</h2>
      <p>
      ...
      <p>
        This certificate covers the Go policy gate only. Java quantitative risk scoring is reported separately in the live case view.
      </p>
...
def sync_case_with_session_state(case: LiveComplianceCase, state: dict) -> None:
    ...
    java_handoff = state.get("java_handoff", {})
    risk_score = state.get("risk_score", {})
    if handoff:
        case.handoff = handoff
    if java_handoff:
        case.java_handoff = java_handoff
    if risk_score:
...
def case_payload(case: LiveComplianceCase) -> dict[str, Any]:
    ...
    return {
        ...
        "java_handoff": case.java_handoff,
